...
import { IconImage, IconTrash, IconAlertCircle } from './icons';
...
export default function ImageUploader({ onUpload, current }) {
  ...
  const upload = async (e) => {
    ...
    try {
      ...
      setError('আপলোড ব্যর্থ: ' + err.message);
    }
    setUploading(false);
  };
  ...
  return (
    <div>
      ...
      {error && (
        <p className="flex items-center gap-1.5 text-rose-500 text-xs mt-2">
          <IconAlertCircle className="w-3.5 h-3.5" /> {error}
        </p>
      )}
    </div>
  );
